            ...
            {cards.map((card, i) => (
                <MagicCard key={i}>
                    <div className="flex h-40 flex-col items-center justify-center border border-border rounded-lg p-4 text-center">
                        <TextAnimate once delay={0.8} className="text-xl font-semibold">{card.title}</TextAnimate>
                        <TextAnimate once animation="slideLeft" by="character" className="text-muted-foreground text-sm" delay={1}>{card.desc}</TextAnimate>
                    </div>
                </MagicCard>
            ))}
        </div>
